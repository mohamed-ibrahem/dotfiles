...
setopt PROMPT_SUBST
PROMPT='%B%{$fg[green]%}Mohamed@%{$fg[green]%}%M %{$fg[blue]%}%~%{$fg[yellow]%}$(git_prompt) %{$reset_color%}
%(?.›.%{$fg[red]%}›)%b '
...
. "${XDG_DATA_HOME}/zsh-artisan/artisan.plugin.zsh"
. "${XDG_DATA_HOME}/zsh-sudo/sudo.plugin.zsh"
